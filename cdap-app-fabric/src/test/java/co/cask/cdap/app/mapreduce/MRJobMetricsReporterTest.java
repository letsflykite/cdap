...
public class MRJobMetricsReporterTest {
  ...
  public void testGetMRJobInfo() throws Exception {
    ...
    gauge(mapTask2Context, MapReduceMetrics.METRIC_TASK_COMPLETION, measureTime, 0L);
    gauge(mapTask2Context, MapReduceMetrics.METRIC_INPUT_RECORDS, measureTime, 0L);
    gauge(mapTask2Context, MapReduceMetrics.METRIC_OUTPUT_RECORDS, measureTime, 0L);
    ...
    Assert.assertEquals((Long) 32L, jobCounters.get(TaskCounter.MAP_INPUT_RECORDS.name()));
    Assert.assertEquals((Long) 320L, jobCounters.get(TaskCounter.MAP_OUTPUT_RECORDS.name()));
    ...
    Assert.assertEquals(0.0F, mapTask2.getProgress(), permittedProgressDelta);
